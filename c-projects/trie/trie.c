...
bool check(char *word)
{
    // set a temporary node to pass from the trie
    ...
    for (int i = 0; i < strlen(word); i++)
    {
        ...
        if (temporaryPointer->children[index] == NULL)
        {
            return false;
        }

        temporaryPointer = temporaryPointer->children[index];
    }
    return temporaryPointer->is_word;
}
